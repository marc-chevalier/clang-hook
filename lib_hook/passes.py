"""Get passes form config and optimization level."""
import typing
...
def get_passes(config: HookConfig, level: OptimisationLevel) -> typing.List[str]:
    """Get passes form config and optimization level.
    Config override optimization level. This way decision of cmake do not change the run passes.
    Passes are hardcoded. They could be get from
    llvm-as < /dev/null | opt -O3 -disable-output -debug-pass=Arguments
    and
    echo 'int;' | clang -xc -O3 - -o /dev/null -\#\#\#
    """
    default = {
        OptimisationLevel.O0: "-targetlibinfo -tti -verify".split(),
    ...
    if config.passes:
        def gen_load(load_list):
            """Generate -load lib arguments from a list of paths."""
            for p in load_list:
                yield "-load"
                yield p
